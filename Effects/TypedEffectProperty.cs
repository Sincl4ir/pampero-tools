...
namespace Pampero.Tools.Effects
{
    ...
    public abstract class TypedEffectProperty<T> : ScriptableObject, ITypedEffectStrategy<T>
        {
            /// <summary>
            ...
            public virtual bool TryToExecute(ref T data, out bool success)
            {
                success = false;

                if (CanExecute(ref data))
                {
                    Execute(ref data, out success);
                    return true;
                }
            ...
            /// </summary>
            /// <param name="data">The data needed for the effect property execution.</param>
            public abstract void Execute(ref T data, out bool success);
    }
}   
//EOF.